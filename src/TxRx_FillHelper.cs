...
// TxRx_FillHelper.cs - returns (Tx, Rx) from the 8 bytes of a screen channel (1..16).
public static class TxRx_FillHelper
{
    // Screen (1..16) → file index (0-based), per DOS view
    ...
    public static (string Tx, string Rx) GetDisplayTones(byte[] image128, int screenCh1to16)
    {
        ...
        // Use existing big-endian aware decoding
        var (tx, rx) = ToneLock.DecodeChannel(A3, A2, A1, A0, B3, B2, B1, B0);
        return (tx, rx);
    }

    // Debug variant: force TX of screen channel 1 to "100.0" (Cg[12])
