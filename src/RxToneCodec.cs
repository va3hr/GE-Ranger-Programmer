...
// GE Rangr Programmer — RX bit-level decode/encode using confirmed mapping.
// This file is self-contained: a banked map, decode, and encode helpers.
// It reads/writes only the exact RX bits and follow/bank flags; leaves other bits untouched.
...
namespace RangrApp.Locked
{
    public static class RxToneCodec
    {
        ...
        /// <param name="A3">Original A3 byte for the channel (preserved except RX bits).</param>
        /// <param name="B3">Original B3 byte for the channel (preserved except follow/bank bits).</param>
        /// <param name="display">Desired RX tone display (e.g., "162.2", "0"). "." is treated as "0".</param>
        /// <param name="follow">Follow-TX flag to write (true/false).</param>
        /// <param name="bank">0 or 1.</param>
        public static (byte newA3, byte newB3) EncodeRxTone(byte A3, byte B3, string display, bool follow, int bank)
        {
            // Convert display → RX index (0 means "0"/no tone).
