...
namespace RangrApp.Locked
{
    public static class ToneLock
    {
        ...
        // Menus used by the UI (no "0" here by design; UI shows "0" as blank)
        ...
        // Index → label map where index 0 is “0”
        ...
        private static string LabelFromIndex(int idx) =>
            (idx >= 0 && idx < ToneIndexToLabel.Length) ? ToneIndexToLabel[idx] : "Err";
        ...
        // Project convention for STE (unchanged)
        public static bool IsSquelchTailEliminationEnabled(byte A3) => ((A3 >> 7) & 1) == 1;

        // ---------------------------------------------------------------------
        // TRANSMIT (MSB→LSB)  — FINAL WINDOW
        //   i5..i0 = [B0.4, B3.1, B2.2, B0.5, B2.4, B2.6]
        //                         ^^^^^  ^^^^^
        //                          i3     i2  ← note i2 = B0.5 (4’s place)
        // ---------------------------------------------------------------------
