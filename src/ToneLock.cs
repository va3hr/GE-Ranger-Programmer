...
namespace GE_Ranger_Programmer
{
    public static class ToneLock
    {
        ...
        /// FIXED: Now uses BigEndian.cs for proper nibble extraction
        /// </summary>
        ...
        private static byte ExtractNibbleFromEEPROM(byte[] logical128, int eepromAddress)
        {
            ...
            // Use BigEndian.cs for proper nibble extraction
            // Even addresses (0x00, 0x02, 0x04...) use upper nibble (Hi)
            // Odd addresses (0x01, 0x03, 0x05...) use lower nibble (Lo)
            return isOddAddress ? 
                (byte)BigEndian.Lo(byteValue) :     // Lower nibble for odd addresses
                (byte)BigEndian.Hi(byteValue);      // Upper nibble for even addresses
        }
