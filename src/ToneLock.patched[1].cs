...
namespace RangrApp.Locked
{
    public static class ToneLock
    {
        ...
        // --------------------------------------------------------------------
        // RX: A3-only index (i5..i0 = [A3.6, A3.7, A3.0, A3.1, A3.2, A3.3]); bank = B3.bit1; Follow= B3.bit0
        // --------------------------------------------------------------------
        ...
        {
            // Legacy shim: ignore A2; use cached last TX so Follow (idx==0 & B3.bit0) can mirror TX
            string txForFollow = "0";
            if (_lastValid)
            {
                string t = TxToneFromBytes(_A1, _B1);
                if (!string.IsNullOrEmpty(t)) txForFollow = t;
            }
            return RxToneCodec.DecodeRxTone(A3, B3, txForFollow);
        }
        ...
        {
            // Preserve current follow/bank flags from B3
            int bank = (B3 >> 1) & 1;
            bool follow = (B3 & 1) != 0;

            // Delegate to RxToneCodec; only A3/B3 are meaningful for RX, A2 is unaffected
            var (newA3, newB3) = RxToneCodec.EncodeRxTone(A3, B3, tone, follow, bank);
            A3 = newA3; B3 = newB3;
            return true;
        }
            ...
            byte A3, byte A2, byte A1, byte A0, byte B3, byte B2, byte B1, byte B0)
        {
            ...
            // RX now decoded via RxToneCodec (A3-only index + Follow/Bank in B3)
