...
using RangrApp.Locked; // ToneLock (TX/RX baked tables + helpers)
...
namespace RangrApp
{
    public partial class MainForm : Form
    {
        ...
        // --- Image store (256 bytes in-memory; 128B legacy is padded/mapped) ---
        ...
        public MainForm()
        {
            ...
            BuildGrid();
            LoadBlankImage();   // defined below; ensures a valid image is in memory
            RefreshAll();
        }
        ...
        private void BuildGrid()
        {
            ...
            var txCol = new DataGridViewComboBoxColumn
            {
                Name = "Tx Tone",
                HeaderText = "Tx Tone",
                Width = 88,
            ...
            var rxCol = new DataGridViewComboBoxColumn
            {
                Name = "Rx Tone",
                HeaderText = "Rx Tone",
                Width = 88,
            ...
            // TX nibbles (E8L, EDL, EEL, EFL) — read-only display
            ...
            // RX nibbles (E0L, E6L, E7L) — read-only display
            ...
            // GE hex view: A0..A3 B0..B3 | A4..A7 B4..B7 (16 bytes)
            _grid.Columns.Add(new DataGridViewTextBoxColumn
            {
                Name = "HEX",
                HeaderText = "A0..A3 B0..B3 | A4..A7 B4..B7",
                Width = 450,
                ReadOnly = true
            });
        }
        ...
        {
            // Row order is the GE layout: A0..A7 (0..7), B0..B7 (8..15)
            var row16 = SliceRow16(ch);

            // --- Frequencies (always compute) ---
            byte A0 = row16[0], A1 = row16[1], A2 = row16[2];
            byte B0 = row16[4], B1 = row16[5], B2 = row16[6];
            double tx = FreqLock.TxMHz(A0, A1, A2);
            double rx = FreqLock.RxMHz(B0, B1, B2, tx); // NOTE: 4th arg is required by your signature
        ...
        // ---------------- Image access (A0..A7 B0..B7 per channel) ----------------

        private void LoadBlankImage()
        {
            _imageIs256 = true;
            _image = new byte[256]; // all 0x00
        }

        private byte[] SliceRow16(int channelIndex)
        {
            // GE/DOS layout: rows at base addresses E0, D0, C0, ... 10 (one 16B row per channel)
            ...
            // Legacy 128B: synthesize 16B row from 8B
            int ch = channelIndex;
            int baseOffset = ch * 8;
            ...
            block16[0x08] = 0x00; // E8 host byte; we only use low nibble for display/preserve
            block16[0x0D] = 0x00; // ED host byte
        ...
        private void StoreRow16(int channelIndex, byte[] row16)
        {
            ...
            // B0..B1 back; B2/B3 carry EE/EF low nibbles
